...
import { textSort } from "@/constants/filters";
...
export const Grouping = () => {
  const [ConfirmDialog, confirm] = useConfirm({
    title: "Would you like to remove Alphabetical group sorting?",
    description: "",
    className: "w-[300px]",
    confirmLabel: "Remove",
  ...
  const headerItems = useMemo(() => {
    const headers = Object.entries(groupingHeaders).filter(([, value]) => value.isShow);
    
    if (groupOption[SORT_KEY]?.value === TextSort.ALPHABETICAL) {
      return headers.map(([key]) => key).sort();
    }
    
    if (groupOption[SORT_KEY]?.value === TextSort.REVERSE_ALPHABETICAL) {
      return headers.map(([key]) => key).sort().reverse();
    }

    return headers
    .sort(([, a], [, b]) => a.order - b.order)
    .map(([key]) => key);
  }, [groupingHeaders, groupOption]);
        ...
        <div className="flex flex-col p-1 shadow-[0_-1px_0_rgba(55,53,47,0.09)]">
          ...
          <div ref={containerRef}>
            <Reorder.Group
              ...
              values={headerItems}
              onReorder={(newOrder) => {
                if (groupOption["Sort"].value === TextSort.MANUAL) {
                  reorderGrouping(newOrder);
                } else {
                  handleConfirm();
                }
              }}
